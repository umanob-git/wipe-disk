...
- **コンソールの「簡易編集モード」（既定で有効）は、枠内をクリックすると選択状態になり、次の出力でプロセスを止める。**
  タイトルバーが「選択」に変わり、進捗が最後の行で固まったまま何分も動かない（2026-09-22、全セクタ検証が 0% で 6 分停止。
  同じ理由で `clean all` 中のウィンドウも止まって見える）。`Esc` で再開する。長時間の実行前に、ウィンドウのプロパティで
  簡易編集モードを切っておく
